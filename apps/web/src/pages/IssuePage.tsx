...
import { CATEGORY_META, STATUSES, canTransition, type Status } from '@amar/shared';
    ...
    <div className="stack" style={{ gap: 20 }}>
      <div>
        ...
        <div className="spread row-wrap" style={{ alignItems: 'flex-start' }}>
          ...
          {isStaff && (
            <button type="button" className="btn primary" onClick={() => setStatusSheet(true)}>
...
function StatusSheet({
  ...
  const [to, setTo] = useState<Status | ''>(legal[0] ?? '');
  ...
  const submit = useMutation({
    mutationFn: async () => {
      ...
      if (to === 'resolved') {
        if (files.length === 0) {
          throw new Error(t({ en: 'A photo of the completed work is required to close a problem.', bn: 'সমস্যা বন্ধ করতে সমাপ্ত কাজের ছবি দিতে হবে।' }));
        }
      ...
      <div className="stack">
        {legal.length === 0 ? (
          ...
          <>
            ...
            {to === 'resolved' && (
              <div className="field">
                <label htmlFor="proof">{t(COPY.proofOfFix)}</label>
                <input
                  id="proof"
                  className="input"
                  type="file"
                ...
                <span className="hint">
                  {t({
                    en: 'Required. A problem cannot be marked resolved on your word alone.',
                    bn: 'আবশ্যক। শুধু আপনার কথায় সমস্যা সমাধান হিসেবে ধরা হবে না।',
                  })}
                </span>
              </div>
            )}
