...
/**
 * Every account needs an email now that it is the sign-in identifier. Seeded
 * people who are not the operator get an address under the reserved `.test`
 * TLD, which by RFC 2606 can never resolve — so nothing here can be mistaken
 * for, or accidentally mail, a real person.
 */
...
async function main() {
  ...
  const signInLines = [
    ...
    ...AUTHORITY_STAFF.map((member, i) =>
      staffEmails[i]
        ? `    Authority  ${staffEmails[i]}  (${member.department})`
        : `    Authority  — no email set —  (${member.department})`,
